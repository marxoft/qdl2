...
void GeneralSettingsPage::restore() {
    ...
    m_concurrentSpinBox->setValue(Settings::instance()->maximumConcurrentTransfers());
    m_commandEdit->setText(Settings::instance()->customCommand());
...
void GeneralSettingsPage::save() {
    Settings::instance()->setDownloadPath(m_pathEdit->text(), false);
    Settings::instance()->setMaximumConcurrentTransfers(m_concurrentSpinBox->value(), false);
...
void GeneralSettingsPage::showFileDialog() {
    const QString path = QFileDialog::getExistingDirectory(this, tr("Download path"), Settings::instance()->downloadPath());
