...
public:
        ...
        QHash<int, QByteArray>()
    {
        ...
        insert(TransferItem::PluginIconRole, "pluginIcon");
...
bool TransferItem::queue() {
    ...
    request->get("/transfers/startTransfers");
...
bool TransferItem::pause() {
    Request *request = new Request(this);
    request->get("/transfers/pauseTransfers");
    connect(request, SIGNAL(finished(Request*)), this, SLOT(pauseRequestFinished(Request*)));
    return true;
}

bool TransferItem::cancel(bool) {
    return false;
}

bool TransferItem::reload() {
    Request *request = new Request(this);
    request->get("/transfers/getTransfers");
    connect(request, SIGNAL(finished(Request*)), this, SLOT(reloadRequestFinished(Request*)));
    return true;
}

void TransferItem::restore(const QVariantMap &) {}

void TransferItem::save() {}

void TransferItem::childItemFinished(TransferItem*) {}

void TransferItem::queueRequestFinished(Request *request) {
...
void TransferItem::startRequestFinished(Request *request) {
    if (request->status() == Request::Finished) {
        reload();
    }
    else if (request->status() == Request::Error) {
...
void TransferItem::pauseRequestFinished(Request *request) {
    if (request->status() == Request::Finished) {
        reload();
    }
    else if (request->status() == Request::Error) {
...
void TransferItem::reloadRequestFinished(Request *request) {
    if (request->status() == Request::Finished) {
        const QVariantList packages = request->result().toList();
        const int diff = packages.size() - rowCount();

        if (diff > 0) {
            for (int i = 0; i < diff; i++) {
                appendRow(new Package(this));
            }
        }
        else if (diff < 0) {
            for (int i = rowCount() - 1; i >= packages.size(); i--) {
                removeRow(i);
            }
        }
        ...
        for (int i = 0; i < packages.size(); i++) {
            if (TransferItem *item = childItem(i)) {
                item->restore(packages.at(i).toMap());
            }
        }
