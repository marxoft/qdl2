...
maemo5 {
    ...
    config.sources = qdl2-vidlox.json
    config.path = !:/qdl2/plugins/services
    
    icon.sources = qdl2-vidlox.jpg
    icon.path = !:/qdl2/plugins/icons

    plugin.sources = qdl2-vidlox.js
    plugin.path = !:/qdl2/plugins/services

    vendorinfo += "%{\"Stuart Howarth\"}" ":\"Stuart Howarth\""
    vidlox_deployment.pkg_prerules += vendorinfo

    DEPLOYMENT.display_name = QDL Plugins Vidlox.tv
    DEPLOYMENT += \
        vidlox_deployment \
        config \
        icon \
