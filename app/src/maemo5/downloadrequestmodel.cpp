...
DownloadRequestList DownloadRequestModel::results() const {
    return m_results;
}
...
QString DownloadRequestModel::resultsString() const {
    ...
    foreach (const DownloadRequest &request, results()) {
        s.append("Filename: ");
        s.append(request.fileName);
        s.append("\nURL: ");
        s.append(request.request.url().toString());
        s.append("\nMethod: ");
        s.append(QString::fromUtf8(request.method));
        s.append("\n");

        if (!request.data.isEmpty()) {
            s.append("Post data: ");
            s.append(QString::fromUtf8(request.data));
            s.append("\n");
        }
        ...
        if (!headers.isEmpty()) {
            ...
            foreach (const QByteArray &header, headers) {
                ...
                s.append(": ");
                s.append(QString::fromUtf8(request.request.rawHeader(header)));
                s.append("\n");
            }
        }
        ...
        case 1:
            if (m_items.at(index.row()).checked) {
                if (m_items.at(index.row()).ok) {
                    return QIcon::fromTheme("widgets_tickmark_grid");
                }
...
void DownloadRequestModel::nextDownloadRequest() {
    if (!m_requests.isEmpty()) {
        setStatus(Active);
        requester()->getDownloadRequest(m_requests.first().url);
    }
    else {
        nextUrlCheck();
    }
...
void DownloadRequestModel::onUrlChecked(const UrlResult &result) {
    ...
    Logger::log(QString("DownloadRequestModel::onUrlChecked(): %1 1 URL found").arg(m_items[m_index].url),
...
void DownloadRequestModel::onDownloadRequest(const QNetworkRequest &request, const QByteArray &method,
    ...
    if ((m_index < 0) || (m_index >= m_items.size()) || (m_requests.isEmpty())) {
        return;
    }

    Logger::log("DownloadRequestModel::onDownloadRequest(): " + request.url().toString(), Logger::MediumVerbosity);
    m_results << DownloadRequest(m_requests.takeFirst().fileName, request, method, data);
...
void DownloadRequestModel::onCheckerError(const QString &errorString) {
    ...
    Logger::log(QString("DownloadRequestModel::onCheckerError(): %1. Error: %2").arg(m_items[m_index].url)
            .arg(errorString));
    m_items[m_index].checked = true;
...
void DownloadRequestModel::onRequesterError(const QString &errorString) {
    ...
    Logger::log(QString("DownloadRequestModel::onRequesterError(): %1. Error: %2").arg(m_requests.takeFirst().url)
            .arg(errorString));
