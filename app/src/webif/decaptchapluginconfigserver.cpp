...
bool DecaptchaPluginConfigServer::handleRequest(QHttpRequest *request, QHttpResponse *response) {
    ...
    if (parts.size() == 1) {
        if (request->method() == QHttpRequest::HTTP_GET) {
            writeResponse(response, QHttpResponse::STATUS_OK,
                          QtJson::Json::serialize(Qdl::getDecaptchaPlugins()));
            return true;
        }
    }
    ...
    if (parts.size() == 2) {
        if (request->method() == QHttpRequest::HTTP_GET) {
            const QVariantMap config = Qdl::getDecaptchaPlugin(parts.at(1));
    ...
    if (parts.last() == "settings") {
        if (request->method() == QHttpRequest::HTTP_GET) {
            writeResponse(response, QHttpResponse::STATUS_OK,
                          QtJson::Json::serialize(Qdl::getDecaptchaPluginSettings(parts.at(1))));
            return true;
        }
        ...
        if (request->method() == QHttpRequest::HTTP_PUT) {
            ...
            if ((properties.isEmpty()) || (!Qdl::setDecaptchaPluginSettings(parts.at(1), properties))) {
