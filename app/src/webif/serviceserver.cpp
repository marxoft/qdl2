...
void ServiceServer::handleRequest(QHttpRequest *request, QHttpResponse *response) {
    ...
    if (method == "getplugins") {
        ...
        if (request->method() == QHttpRequest::HTTP_GET) {
            // OK
            const QVariantList plugins = Qdl::getServicePlugins();
        ...
        if (request->method() == QHttpRequest::HTTP_GET) {
            ...
            if (!id.isEmpty()) {
                const QVariantMap plugin = Qdl::getServicePlugin(id);
        ...
        if (request->method() == QHttpRequest::HTTP_GET) {
            ...
            if (!id.isEmpty()) {
                // OK
                const QVariantList settings = Qdl::getServicePluginSettings(id);
        ...
        if (request->method() == QHttpRequest::HTTP_PUT) {
            ...
            if (!id.isEmpty()) {
                ...
                if (Qdl::setServicePluginSettings(id, settings)) {
                    // OK
                    response->writeHead(QHttpResponse::STATUS_OK);
                    response->end();
                    return;
