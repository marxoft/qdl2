...
void RecaptchaServer::handleRequest(QHttpRequest *request, QHttpResponse *response) {
    ...
    if (method == "getplugins") {
        ...
        if (request->method() == QHttpRequest::HTTP_GET) {
            // OK
            const QVariantList plugins = Qdl::getRecaptchaPlugins();
        ...
        if (request->method() == QHttpRequest::HTTP_GET) {
            ...
            if (!id.isEmpty()) {
                const QVariantMap plugin = Qdl::getRecaptchaPlugin(id);
        ...
        if (request->method() == QHttpRequest::HTTP_GET) {
            ...
            if (!id.isEmpty()) {
                // OK
                const QVariantList settings = Qdl::getRecaptchaPluginSettings(id);
        ...
        if (request->method() == QHttpRequest::HTTP_PUT) {
            ...
            if (!id.isEmpty()) {
                ...
                if (Qdl::setRecaptchaPluginSettings(id, settings)) {
                    // OK
                    response->writeHead(QHttpResponse::STATUS_OK);
                    response->end();
                    return;
